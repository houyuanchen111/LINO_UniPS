...
class TrainDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.debug_or_not = debug_or_not
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            self.train_dataset = TrainData(
                mode='Train',    # 通常是 'Train'
                data_root=self.data_root,
                debug_or_not=self.debug_or_not,
                low_normal=self.low_normal
            )
            self.val_dataset = TrainData(
                mode='Val',     # 通常是 'Test' or 'Val'
                data_root=self.data_root,
                debug_or_not=self.debug_or_not,
                low_normal=self.low_normal
            )
        if stage == "test" or stage is None:
            self.test_dataset = TrainData(
                mode="Test",    # 'Test'
                data_root=self.data_root,
                debug_or_not=self.debug_or_not,
                low_normal=self.low_normal,
                numMaxImages=self.numMaxImages
            )
